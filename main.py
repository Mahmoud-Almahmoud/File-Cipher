...
SALT_SIZE = 16
BLOCK_SIZE = 16
CHUNK_SIZE = 64 * 1024  # 64KB Read/Write buffer size
...
# ---- Background Worker Logic (Optimized for Large Media Streams) ----
def file_worker(file_path, password, encrypt=True):
    try:
        ...
        if encrypt:
            ...
            # REMOVED ZLIB STREAM COMPRESSION: Prevents pipeline corruption on large files
            with open(file_path, "rb") as f_in, open(out_path, "wb") as f_out:
                f_out.write(salt + iv) # Write 32-byte tracking header
                ...
                while True:
                    ...
                    # Pass the raw chunk straight to AES-CTR
                    encrypted_chunk = encryptor.update(chunk)
                    f_out.write(encrypted_chunk)
                    
                    bytes_processed += len(chunk)
                    # Safe thread-throttled progress updates
            ...
            if total_size < (SALT_SIZE + BLOCK_SIZE):
                raise Exception("File is too small to contain valid encrypted metadata.")

            with open(file_path, "rb") as f_in, open(out_path, "wb") as f_out:
                # Isolate the explicit 32-byte tracking header
...
# Progress Row
progress_bar = ctk.CTkProgressBar(main_frame, width=370, progress_color="#2ECC71",fg_color="#2C3E50")
...
decrypt_btn = ctk.CTkButton(btn_frame, text="Decrypt File", fg_color="#34DB37", hover_color="#2ECC71", width=140, font=("Arial", 13, "bold"), command=lambda: start_processing(encrypt=False))
decrypt_btn.pack(side="left")

dev_label = ctk.CTkLabel(main_frame, text="www.mahmoudev.com", font=("Arial", 12, "italic"), text_color="white")
